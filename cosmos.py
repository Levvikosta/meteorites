...
while is_running:
    ...
    if mode == "meteorites":
        ...
        # Отрисовка очков
        score_text = font_small.render(f"Очки: {points}", True, WHITE)
        ...
        score_text = font_medium.render(f"Вы набрали очков: {points}", True, WHITE)
        score_rect = score_text.get_rect(center=(size[0] // 2, size[1] // 2 - 30))
        screen.blit(score_text, score_rect)

        # Подсказка о перезапуске
        restart_text = font_medium.render("Нажмите R для перезапуска ", True, GREEN)
        restart_rect = restart_text.get_rect(center=(size[0] // 2, size[1] // 2 + 40))
        screen.blit(restart_text, restart_rect)

        # Подсказка о выходе
        exit_text = font_small.render("Нажмите ESC чтобы выйти", True, GREY)
